...
import { createDate, createValidObjectFromJSON } from "../helpers/helpers";
...
export const Posts = (props) => {
  ...
  const fetchPosts = async () => {
    let postsDB = [];
    let catsDB = [];
    let newPosts = await getPosts().then((posts) => {
  ...
  return (
    <div style={{ display: "flex" }}>
      <Box
        style={PostsCSS}
        component="div"
        sx={{ marginTop: "50px", height: "400px", borderRadius: "5px" }}
      >
        {!filteredPosts ? (
              ...
              posts.map((p, i) => {
                ...
                return (
                  <div id="post-container" key={blogObj.title}>
                    ...
                    <Link to={`/blog/${blogObj.id}`} className="title-link">
                      <Typography
                        style={{ marginTop: "25px", fontWeight: "200" }}
                        variant="h5"
                        key={p + i}
                      >
                        {blogObj.title}
              ...
              filteredPosts.map((p, i) => {
                ...
                return (
                  <div id="post-container" key={p.title}>
                    <Link to={`/blog/${p.id}`} className="title-link">
                      <Typography
                        style={{ marginTop: "25px", fontWeight: "200" }}
                        variant="h5"
                        key={p + i}
